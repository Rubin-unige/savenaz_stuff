...
#define DRIVER_PATH "/dev/specific"
...
main()
{
	...
	//if not, you will need to sort them
  	periods[0] = 300000000; // 300 ms
        periods[1] = 500000000; // 500 ms
        periods[2] = 800000000; // 800 ms
...
// Application-specific task 1 code (wasting time)
void task1_code() {
...
// Application-specific task 2 code (wasting time)
void task2_code() {
...
// Application-specific task 3 code (wasting time)
void task3_code() {
    int fd;
    // open driver
    fd = open(DRIVER_PATH, O_WRONLY);
...
// Aperiodic task triggered by J2
void task4_code() {
    int fd;
    // open driver
    fd = open(DRIVER_PATH, O_WRONLY);
...
// Thread function for aperiodic task
void *task4(void *ptr) {
    cpu_set_t cset;
    CPU_ZERO(&cset);
    CPU_SET(0, &cset);
    ...
    while (1) {
        // Wait for task J2 to trigger task J4
